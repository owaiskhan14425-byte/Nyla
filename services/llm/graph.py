...
# The above code is a Python script that defines a graph structure for a language model (LLM) with
# tools. Here is a summary of what the code does:
...
# from langchain_openai import ChatOpenAI
# from langchain_core.messages import SystemMessage, HumanMessage, AIMessage
# from services.llm.tools import TOOLS, rag_tool, transaction_tools
# import os
# from dotenv import load_dotenv
...
#                     "   Do NOT address the user or change the speaker’s identity.\n"
#                     "4) If the reply includes structured data (client codes, IDs, JSON, lists, tables, bullets): "
#                     "   copy that DATA verbatim and only trim surrounding prose. Do NOT truncate, mask, or summarize the data itself. "
#                     "   It is allowed for the final output to approach the word limit due to data blocks.\n"
#                     "5) If the reply appears incomplete or cut off, complete only the current sentence "
#                     "   so that it becomes grammatically complete — do not add new information.\n"
#                     "\n"
#                     "Output: plain text only, no meta commentary."
...
#     # Tool nodes
#     builder.add_node("transaction", ToolNode(transaction_tools))   # search_client, all_investment, scheme_details, purchase, etc.
#     builder.add_node("rag", ToolNode([rag_tool]))            # single rag tool
#     builder.add_node("tools", ToolNode(tools=))             # catch-all for parallel tool calls

#     # Edges
#     builder.add_edge(START, "llm")

#     # Build routing map dynamically for all transaction tools
#     # (avoids hardcoding and future misses)
#     try:
#         txn_map = {t.name: "transaction" for t in transaction_tools}
#     except Exception:
#         # Fallback: in case any tool object lacks .name (shouldn't happen with @tool)
#         txn_map = {}
#         for t in transaction_tools:
#             name = getattr(t, "name", None) or getattr(t, "__name__", None)
#             if name:
#                 txn_map[name] = "transaction"

#     route_map = {
#         # Specific routes
#         "rag_tool": "rag",

#         # Add all transaction tools
#         **txn_map,

#         # Catch-all when LLM emits parallel tool calls (sentinel from tools_condition)
#         "tools": "tools",
...
#     builder.add_conditional_edges("llm", tools_condition, route_map)

#     # Tools loop back to LLM
#     builder.add_edge("transaction", "llm")
