...
async def robot_ask_llm(session_id, question, user_info=None, openai_key=None):
   ...
   # seed (system only for brand-new thread)
   if prev_len > 0:
   ...
   # final answer with fallback (turn -> all)
   final_text = _last_ai_text(turn_msgs) or _last_ai_text(all_msgs)

   # tools used THIS turn + label
   ...
   print_graph_history(session_id, graph)
   print("="*50)
